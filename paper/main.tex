...
% the 2026-06-12 codex review re-derived them from raw logs (CODEX_QUEUE_
% FINDINGS.md ~L909). xlayer_rung0/ was deleted in a July disk cleanup AFTER
...
\noindent\emph{Proof sketch.} For the fractional relaxation the greedy
ascending-$\rho$ fill is exact, so $D(R)$ is the integral of a nondecreasing step
function --- convex, and affine exactly when $\rho_{\mathrm{sorted}}$ is flat. The
integer optimum is feasible for the relaxation, hence weakly above the envelope;
with unequal saved-bit sizes $\Delta b_i$ the two need not coincide except in the
limit of fine granularity. \hfill$\square$

\begin{proposition}[The knee tracks the cost-per-bit density, not raw sensitivity]\label{prop:knee}
The curvature of $D(R)$ is governed by the dispersion of $\{\rho_i\}$, not of the
raw sensitivities $\{c_i\}$. With a single lossy format the saved bits scale with
size and $\rho_i\propto c_i/n_i$, which on our models gives a \emph{smooth} convex
curve. A \emph{sharp} knee requires a strong gap in the sorted-$\rho$ density;
such a gap is \emph{sufficiently} produced by an intermediate-efficiency rung
(e.g.\ FP8, for which BF16$\to$FP8 buys many bits at near-zero cost while
FP8$\to$NVFP4 is expensive), which a single-lossy menu cannot supply. A
sufficiently heavy-tailed $\rho$ alone could also produce one; we do not observe
that on the two-rung menu here.
\end{proposition}

\paragraph{Consequences, on real data.} On the Qwen3.6-27B
\textsc{Aura} costs, the cost-per-bit is far from democratic: weighted Gini
...
``the two-rung frontier looks linear'' are simultaneously true. We therefore
